"""The renewals HTTP surface: the on-demand sweep endpoints (P2.4 Epic 6).

`POST /api/renewals/aep-sweep` is the thinnest customer-visible slice of the renewals
thread: a **Platform Admin** button fires one AEP renewal sweep over the caller's demo
session in their currently-selected tenant, and gets back `{generated, skipped}`. The
sweep bypasses the Oct 15 – Dec 7 seasonal calendar (the on-demand button is always in
season, ADR 0004) and writes no audit record — the outbox event trail is the record
(ADR 0006), mirroring the sibling `reset_demo_session`.
...
async def aep_sweep(
    ...
    `403 "insufficient permissions"` paths). It then resolves the caller's demo
    session and the tenant it last selected, and runs `generate_renewals` for the
    `"aep"` rule inside a scoped tenant write session:
