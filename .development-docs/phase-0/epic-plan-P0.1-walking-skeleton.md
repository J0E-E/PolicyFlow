...
## Epic 8 — Terraform ECR + CodeBuild image build — **COMPLETED**
...
- **Implementation notes:**
  - **ECR (`infra/ecr.tf`):** one repo per image — `policyflow-core`, `policyflow-frontend` — via `for_each` over a `local.ecr_repositories` set (matches Epic 1 + the `${project_name}-*` ARN prefix the host's ECR-pull policy is scoped to). Each repo is `image_tag_mutability = "MUTABLE"` (so the moving `:latest` tag can be re-pointed) with `scan_on_push = true`. An `aws_ecr_lifecycle_policy` (`for_each` over the created repos) expires **untagged** images after 14 days — tagged `:<short-sha>`/`:latest` images are kept.
  - **Buildspec (`ops/buildspec.yml`, `version: 0.2`):** `pre_build` derives `SHORT_SHA` = first 7 chars of `CODEBUILD_RESOLVED_SOURCE_VERSION` (fallback `dev`) and logs Docker in to ECR; `build` runs `docker build` for `core` (with `--build-arg GIT_SHA=$SHORT_SHA`, baked into `APP_VERSION`) and `frontend` (no GIT_SHA arg), each tagged `:$SHORT_SHA` + `:latest`; `post_build` runs `docker push --all-tags` per repo. Reads `CORE_REPOSITORY_URL` / `FRONTEND_REPOSITORY_URL` / `AWS_DEFAULT_REGION` from CodeBuild env vars, so no account id is hard-coded.
  - **CodeBuild (`infra/codebuild.tf`):** `aws_iam_role.build` trusted by `codebuild.amazonaws.com`, with two inline policies mirroring `iam.tf`'s ARN-prefix style — ECR push (`GetAuthorizationToken` on `*`; push/pull actions scoped to `repository/${project_name}-*`) and CloudWatch Logs (`CreateLogGroup`/`CreateLogStream`/`PutLogEvents` scoped to the `/aws/codebuild/${project_name}-build` group, with the `:*` stream-ARN variant needed for `PutLogEvents`). `aws_codebuild_project.build` (`policyflow-build`): `BUILD_GENERAL1_SMALL`, `aws/codebuild/amazonlinux2-x86_64-standard:5.0`, `privileged_mode` (Docker), `NO_ARTIFACTS`, `source { type = "GITHUB", location = var.source_repository_url, buildspec = "ops/buildspec.yml", git_clone_depth = 1 }`, CloudWatch `logs_config`, and the two repo URLs + region as `environment_variable`s.
  - **Variable + tfvars:** added required `source_repository_url` (no default — same safety pattern as `ssh_ingress_cidr`); `terraform.tfvars.example` gains a placeholder GitHub clone URL.
  - **Outputs:** `ecr_repository_urls` (map name → `repository_url`) and `codebuild_project_name`.
  - **Docs:** `infra/README.md` documents `ecr.tf`/`codebuild.tf`/the new variable/outputs, moves ECR+CodeBuild out of "Not here yet", and adds the manual proof (`import-source-credentials` one-time, then `aws codebuild start-build --project-name policyflow-build`, verify with `aws ecr list-images`). `ops/README.md` now documents `buildspec.yml` (appspec still Epic 11).
  - **Deviations from the plan:** (1) added a second `:*` resource ARN to the Logs policy so `PutLogEvents` (which targets the log *stream*, not the group) is actually permitted — the plan named only the group ARN. (2) pinned the CodeBuild image to the concrete `aws/codebuild/amazonlinux2-x86_64-standard:5.0` (the plan said "AWS-managed standard Linux image" without naming a tag). Both x86_64, matching the host arch.
  - **Verification (local, no apply — apply needs credentials and costs money):** `terraform fmt -recursive infra/` clean (no reformatting); `cd infra && terraform init -backend=false && terraform validate` → "The configuration is valid."; `ops/buildspec.yml` parses as valid YAML; `infra/bootstrap` untouched (`git status --porcelain` empty). No `plan`/`apply` run — the README documents the author's manual build proof.

## Epic 9 — Terraform CodePipeline + CodeDeploy wiring — **COMPLETED**
...
- **Depends on:** Epic 8.
- **Implementation notes:**
  - **Phase 1 — artifact bucket + connection (`infra/artifacts.tf`, `infra/codepipeline.tf`, `infra/variables.tf`):** `aws_s3_bucket.pipeline_artifacts` = `${project_name}-pipeline-artifacts-${account_id}` (account id from `data.aws_caller_identity.current` for global uniqueness), hardened like the bootstrap bucket (versioning Enabled, AES256 SSE, public-access-block all-true) + a 30-day object-expiration lifecycle rule. `aws_codestarconnections_connection.github` (`provider_type = "GitHub"`) is created PENDING — the console authorization is the sanctioned manual bootstrap (documented). Added required `github_repository_id` (`owner/repo`, no default — same safety pattern as `ssh_ingress_cidr`/`source_repository_url`; distinct from the `source_repository_url` clone URL) and `github_branch` (default `"main"`).
  - **Phase 2 — CodeDeploy + S3 grants (`infra/codedeploy.tf`, edit `infra/iam.tf`, edit `infra/codebuild.tf`):** `aws_codedeploy_app.app` (`compute_platform = "Server"`) + `aws_codedeploy_deployment_group.host` targeting the host by `Name = ${project_name}-host` EC2 tag filter, `CodeDeployDefault.AllAtOnce`, in-place (`WITHOUT_TRAFFIC_CONTROL` / `IN_PLACE`), no ASG/ELB. `aws_iam_role.deploy` (trust `codedeploy.amazonaws.com`) + inline policy `ec2:Describe*` + `tag:GetResources`/`tag:GetTags` on `*` (discovery cannot be resource-scoped). `iam.tf` header comment updated and the deferred host S3-read grant added (`s3:GetObject`/`GetObjectVersion` on the artifact bucket `.../*` — the CodeDeploy agent downloads the bundle). `codebuild.tf` gains a third inline policy: artifact-bucket `GetObject`/`GetObjectVersion`/`PutObject` on `.../*` + `GetBucketLocation` on the bucket (pipeline-mode source-artifact download).
  - **Phase 3 — pipeline + service role (`infra/codepipeline.tf`):** `aws_iam_role.pipeline` (trust `codepipeline.amazonaws.com`) + inline ARN-scoped policy (artifact-bucket read/write, `codestar-connections:UseConnection` on the connection ARN, `codebuild:StartBuild`/`BatchGetBuilds` on the build project ARN, `codedeploy:CreateDeployment`/`Get*`/`RegisterApplicationRevision` on the app + deployment-group + deployment-config ARNs). `aws_codepipeline.pipeline` (`${project_name}-pipeline`): Source (`CodeStarSourceConnection`, output `source_output`) → Build (reuses `aws_codebuild_project.build`, input `source_output`, **no output artifact** — product is ECR images) → Deploy (CodeDeploy app + group, input **`source_output`**). v2 connection auto-triggers on push — no webhook resource.
  - **Phase 4 — outputs, tfvars, README:** added outputs `github_connection_arn`, `codepipeline_name`, `codedeploy_app_name`, `codedeploy_deployment_group_name`, `artifact_bucket_name`; `terraform.tfvars.example` gains a `github_repository_id` placeholder + commented `# github_branch`; README scope bumped to Epics 6–9, new-file bullets added, pipeline/deploy moved out of "Not here yet", and an "Authorize the GitHub connection (one-time)" section added.
  - **Decisions honored (no deviation):** custom inline ARN-scoped IAM (not AWS-managed); **wire only** — no `ops/appspec.yml`, Deploy stays red until Epic 11; Build reuses the existing CodeBuild project with no output artifact; Deploy input is `source_output`.
  - **Minor note:** deployment-group/deployment-config ARNs in the pipeline policy are constructed as literal `arn:aws:codedeploy:...` strings (no first-class ARN attribute on those resources), using `var.region` + `data.aws_caller_identity.current.account_id` like the existing `iam.tf`/`codebuild.tf` policies.
  - **Sizing:** ~487 insertions / 10 files, over the ~150-line/~8-file budget, but it is all Terraform config (explicitly discounted by the conventions) and the CI/CD trigger path (connection + pipeline + deploy group + roles) is one atomic unit — splitting would ship a half-wired pipeline. Same precedent as Epic 6 (kept whole, justified).
  - **Verification (local, no apply):** `terraform fmt -recursive infra/` clean (no reformatting); `cd infra && terraform init -backend=false && terraform validate` → "The configuration is valid." `infra/bootstrap` untouched (`git status --porcelain infra/bootstrap` empty).

## Epic 10 — Route 53 record + certbot TLS at nginx — **COMPLETED**
...
- **Implementation notes:** advanced — issuance + renewal + DNS + TLS termination are a single correctness-critical unit; splitting would ship a half-configured cert. Human-approved exception.
  - **Phase 1 — Route 53 (`infra/route53.tf`):** `data "aws_route53_zone" "primary"` (`name = var.hosted_zone_name`, `private_zone = false`) reads the **existing** zone, never creates it. `aws_route53_record.host`: `type = "A"`, `ttl = 300`, `records = [aws_eip.host.public_ip]` — a plain EIP is a literal IPv4, so a simple A record (not ALIAS). No SG change (Epic 6 already opens 80 + 443). `variables.tf` gains `hosted_zone_name` (default `joeyshub.com`) + `domain_name` (default `policyflow.joeyshub.com`); `outputs.tf` gains `app_url` (`https://${domain_name}`); `terraform.tfvars.example` gets the two commented defaults.
  - **Phase 2 — prod TLS nginx (`frontend/nginx.tls.conf`, mounted not baked):** `:80` server serves `/.well-known/acme-challenge/` from `/var/www/certbot` and `return 301 https://$host$request_uri` for everything else; `:443 ssl` server points `ssl_certificate`/`_key` at `/etc/letsencrypt/live/policyflow.joeyshub.com/{fullchain,privkey}.pem`, includes certbot's `options-ssl-nginx.conf` + `ssl-dhparam.pem`, and reuses the dev SPA `try_files` + `/api/` resolver-plus-`set $core_upstream` deferred-DNS pattern. Domain hardcoded (committed prod file).
  - **Phase 2/3 — overlay (`docker-compose.prod.yml`, new root file):** `frontend` override adds `ports 80+443`, mounts `nginx.tls.conf` over `default.conf:ro` plus `letsencrypt:/etc/letsencrypt:ro` + `certbot-webroot:/var/www/certbot:ro`, overrides the healthcheck to `wget --no-check-certificate --spider https://127.0.0.1/` (the `:80` check now 301s), and overrides `command` to the 6h `nginx -s reload` loop + `nginx -g 'daemon off;'`. New `certbot` sidecar (`certbot/certbot`) runs the `trap exit TERM; … certbot renew --webroot … sleep 12h` loop — the no-host-cron renewal seam. Two named volumes declared. Base `docker-compose.yml` left HTTP-only and untouched.
  - **Phase 4 — issuance + env + docs:** new `ops/init-letsencrypt.sh` (one-time host bootstrap: reads `CERTBOT_*` from `.env`, downloads certbot's TLS options into the volume, dummy self-signed cert → nginx up → delete dummy → `certbot certonly --webroot` with `--staging` when `CERTBOT_STAGING=1` → reload). `.env.example` gains `CERTBOT_EMAIL`/`CERTBOT_DOMAIN`/`CERTBOT_STAGING`. `ops/README.md` documents the bootstrap; `infra/README.md` documents `route53.tf`, the TLS issuance+renewal flow, and the ACM+ALB written-only fallback (Risk #2 — no ALB built). `*.sh` already pinned to LF in `.gitattributes` (no change).
  - **Decisions/notes:** `init-letsencrypt.sh` is `#!/bin/sh` POSIX (consistent with the LF-pinned entrypoint), reads vars via `grep`/`cut` from `.env` rather than sourcing it (avoids leaking unrelated vars). certbot run via `docker compose run --rm` so it shares the named volumes. `--no-eff-email --non-interactive` added to `certbot certonly` for a hands-off run (sensible defaults, not in the plan literal). `docker-compose.prod.yml` loop bodies use `$${!}` so compose passes a literal `${!}` to the shell at runtime.
  - **Verification (local, no apply, no live cert):** `terraform fmt -recursive infra/` clean (no changes); `cd infra && terraform init -backend=false && terraform validate` → "The configuration is valid."; `docker compose -f docker-compose.yml -f docker-compose.prod.yml config` parses (rendered loops keep the literal `${!}`); `nginx -t` of `frontend/nginx.tls.conf` in a throwaway `nginx:1.27-alpine` container → "syntax is ok / test is successful" (cert files absent at parse time, as expected). The live host proof (run `init-letsencrypt.sh` → real cert → `https://policyflow.joeyshub.com` valid) is deferred to Epics 11–12; `infra/bootstrap` untouched. No `terraform apply` and no real cert issued.

## Epic 11 — CodeDeploy appspec + migrate/seed-on-deploy — **COMPLETED**
...
- **Implementation notes:** kept whole — a half-wired appspec ships nothing; all CI/CD config + shell glue (conventions discount ops/config). Stakeholder decisions confirmed in the plan: pull `:latest`; compose `.env` = committed non-secret defaults + two SSM passwords; hook order AfterInstall → ApplicationStart → ValidateService; migrate/seed reuse Epic 3's core entrypoint (no separate step); no DB reset (named volume persists, migrations idempotent).
  - **Phase 1 — prod image refs (`docker-compose.prod.yml`):** added `core.image: ${CORE_IMAGE}` and `frontend.image: ${FRONTEND_IMAGE}` to the overlay so the host pulls from ECR. Base `docker-compose.yml` left untouched (local dev still builds); the deploy hook `pull`s first so `up -d` uses the pulled image and never builds on the host.
  - **Phase 2 — committed prod defaults (`ops/deploy/prod.env.defaults`):** NON-secret operational values only (`POSTGRES_USER`, `POSTGRES_DB`, `RABBITMQ_DEFAULT_USER`, `CERTBOT_EMAIL=admin@joeyshub.com` domain alias, `CERTBOT_DOMAIN`, `CERTBOT_STAGING`). Passwords + image refs appended at deploy time, never here.
  - **Phase 3 — hook scripts (`ops/deploy/`), all `#!/bin/sh` POSIX, `set -eu`, LF:** `lib.sh` shared seam (`APP_DIR=/opt/policyflow`, `COMPOSE_FILES`, region via IMDSv2 token, account via STS, derived `REGISTRY`/`CORE_IMAGE`/`FRONTEND_IMAGE` at `:latest`) — sourced, not executed. `after_install.sh` writes `/opt/policyflow/.env` from defaults + two SSM SecureStrings (`--with-decryption`) + image refs (`chmod 600`), ECR login, `compose pull`. `application_start.sh` `compose up -d` (migrate+seed run inside core's entrypoint). `validate_service.sh` polls core `/api/health` from inside the container (core publishes no host port) for ~2 min, exits non-zero on failure.
  - **Phase 4 — `appspec.yml` at repo root:** `version: 0.0`, `os: linux`, install `/` → `/opt/policyflow`, `file_exists_behavior: OVERWRITE` (preserves the gitignored deploy-written `.env`), hooks map the three scripts. No `ApplicationStop` (first deploy has no prior revision; `up -d` recreates changed containers).
  - **Phase 5 — exec bit + docs:** all four `ops/deploy/*.sh` set `100755` in the git index (`git update-index --chmod=+x`, confirmed via `git ls-files -s`). `ops/README.md`: added the Epic 11 section (appspec, hooks, defaults, SSM→`.env` composition, bootstrap order) and removed the "Not here yet" appspec bullet.
  - **Decisions/deviations:** hook scripts source `lib.sh` via `. "$(dirname "$0")/lib.sh"` so region/account/registry are derived once (not in the plan literal but the obvious shared-seam form). Added per-hook `timeout`/`runas: root` in appspec (sensible CodeDeploy defaults, not spelled out in the plan). `ValidateService` probes via `compose exec -T core curl …/api/health` (the plan's first option) rather than parsing `compose ps` health state.
